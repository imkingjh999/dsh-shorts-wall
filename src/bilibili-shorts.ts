...
function mapSearchEntry(raw: Record<string, unknown>):
  | {
      bvid: string;
      title: string;
      authorName: string;
...
export class BiliShortsFeed {
  ...
  private async getJson(url: string): Promise<Record<string, unknown>> {
    ...
    if (code !== 0)
      throw new BiliShortsError(`B 站接口错误：${str(body["message"]) ?? `code ${code}`}`);
  ...
  async page(keyword: string, page: number): Promise<{ items: BiliShort[]; hasMore: boolean }> {
    ...
    await Promise.all(
      Array.from(
        {
          length: Math.min(PREFLIGHT_CONCURRENCY, Math.max(entries.length, 1)),
        },
        worker,
      ),
    );
    return {
      items: out,
      hasMore: Array.isArray(list) && list.length >= SEARCH_PAGE_SIZE,
    };
  }
  ...
  next(list: { query: string; region: string }[]): {
    query: string;
    region: string;
  } {
