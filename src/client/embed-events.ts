...
function posterOf(ref: React.RefObject<HTMLIFrameElement | null>): (obj: Record<string, unknown>) => void {
  return (obj) => {
...
export function useYtEmbedEvents(
  ...
  videoKey: string,
  handlers: { onEnded: () => void; onError: (code: number) => void; onAliveChange?: (alive: boolean) => void },
): EmbedEvents {
  ...
  useEffect(() => {
    ...
    const applyState = (state: number): void => {
      if (state === 0) { setPlaying(false); advance() } // ENDED
      if (state === 1) { // PLAYING
        setStarted(true)
        setPlaying(true)
        lastLifeRef.current = Date.now()
    ...
    const onMessage = (e: MessageEvent): void => {
      ...
      try { data = JSON.parse(e.data) as Record<string, unknown> } catch { return }
  ...
  const setMuted = useCallback((muted: boolean): void => {
    posterOf(iframeRef)({ event: 'command', func: muted ? 'mute' : 'unMute', args: [] })
  }, [iframeRef])
