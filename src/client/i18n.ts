...
const zh: Record<string, string> = {
  ...
  "header.soundOn": "声音开",
  "header.soundOff": "已静音",
  "header.muteKeyTip": "静音/开启声音（Alt+M）",
  "header.keywords": "关键词",
  "header.pickKeyword": "选择关键词",
  "shell.open": "打开 Shorts",
  "shell.close": "收起窗口",
  "shell.minimize": "最小化（Alt+S 还原）",
  "shell.floatText": "浮动",
  "shell.modeToggleTip": "开启为浮窗，关闭为贴边（默认贴边）",
  "shell.minimizeText": "最小化",
  "shell.expand": "展开 Shorts",
  "shell.bossKey": "老板键 Alt+S",
  "shell.bossKeyTip": "老板键：Alt+S 最小化 / 还原",
...
const en: Record<string, string> = {
  ...
  "header.muteKeyTip": "Mute/unmute (Alt+M)",
  "header.keywords": "Keywords",
  "header.pickKeyword": "Pick a keyword",
  "shell.open": "Open Shorts",
  "shell.close": "Close window",
  "shell.minimize": "Minimize (Alt+S to restore)",
  "shell.floatText": "Float",
  "shell.modeToggleTip": "On = floating window; off = docked (default)",
  "shell.minimizeText": "Minimize",
  "shell.expand": "Expand Shorts",
  "shell.bossKey": "Boss key Alt+S",
  "shell.bossKeyTip": "Boss key: Alt+S minimizes / restores",
...
export function useT(): (key: string, ...args: unknown[]) => string {
  ...
  useEffect(() => {
    ...
    const off = service.subscribe(() => {
      setRevision(service?.getLocale().revision ?? 0);
    });
    return () => {
      off();
    };
