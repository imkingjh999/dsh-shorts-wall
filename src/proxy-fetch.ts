  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('connect', (res: IncomingMessage, socket) => {
      ...
      const tls = https.request({
        ...
        timeout: timeoutMs,
      } as unknown as https.RequestOptions, (tres) => {
        const chunks: Buffer[] = []
        tres.on('data', chunk => { chunks.push(Buffer.from(chunk)) })
        tres.on('error', fail)
        tres.on('end', () => {
      ...
      tls.on('timeout', () => { tls.destroy(); fail(new Error('tls timeout')) })
      tls.on('error', fail)
      tls.end()
    })
    req.end()
 ...
 */
export function createProxyFetch(proxyUrlRaw: string, timeoutMs: number): (url: string, init?: TunnelInit) => Promise<TunnelResponse> {
  const proxyUrl = new URL(proxyUrlRaw)
  return async (url, init = {}) => {
    let target = new URL(url)
    if (target.protocol !== 'https:') {
      return await fetch(url, { headers: init.headers, redirect: init.redirect, signal: init.signal }) as TunnelResponse
    }
    for (let hop = 0; hop <= REDIRECT_LIMIT; hop++) {
      const response = await tunnelOnce(proxyUrl, target, init.headers ?? {}, init.signal, timeoutMs)
      const location = response.headers.get('location')
      if (init.redirect !== 'follow' || response.status < 300 || response.status >= 400 || location === null) {
        return response
      }
