...
export function collectShortsEntries(node: unknown, out: Map<string, YtVideo> = new Map()): Map<string, YtVideo> {
  ...
  if (typeof lockup === 'object' && lockup !== null) {
    ...
    const onTap = (l['onTap'] as Record<string, unknown> | undefined)?.['innertubeCommand'] as Record<string, unknown> | undefined
    ...
    if (videoId !== undefined && VIDEO_ID.test(videoId) && !out.has(videoId)) {
      ...
      const title = firstRunsText(overlay?.['primaryText'])
        ?? (typeof l['accessibilityText'] === 'string' ? l['accessibilityText'].split(',')[0] : undefined)
      if (title !== undefined) {
        const thumb = Array.isArray(endpoint?.['thumbnail'] as unknown)
          ? ((endpoint!['thumbnail'] as Record<string, unknown>)['thumbnails'] as unknown[])
          : undefined
        const thumbUrl = typeof (thumb?.[0] as Record<string, unknown> | undefined)?.['url'] === 'string'
          ? (thumb![0] as Record<string, unknown>)['url'] as string
  ...
  if (typeof reel === 'object' && reel !== null) {
    ...
    const videoId = typeof r['videoId'] === 'string' && VIDEO_ID.test(r['videoId']) ? r['videoId'] : undefined
 ...
 */
export function collectVideoRenderers(node: unknown, out: Map<string, YtVideo> = new Map()): Map<string, YtVideo> {
  if (Array.isArray(node)) {
  ...
  if (typeof node !== 'object' || node === null) return out
  const record = node as Record<string, unknown>
  if (typeof record['videoId'] === 'string' && VIDEO_ID.test(record['videoId']) && !out.has(record['videoId'])) {
    const title = firstRunsText(record['title']) ?? firstRunsText(record['headline'])
    const author = firstRunsText(record['ownerText']) ?? firstRunsText(record['longBylineText']) ?? str(record['shortBylineText'])
...
export class YtFeed {
  ...
  async page(query: string, options?: { shorts?: boolean }): Promise<{ items: YtVideo[]; hasMore: boolean; query: string }> {
    ...
    const response = await this.fetchImpl(url, {
      ...
      throw new YtError('连不上 YouTube（本机网络需要能直达 YouTube，或给插件配 resolveProxyUrl 代理出口）')
    })
    ...
    const items = options?.shorts === true
