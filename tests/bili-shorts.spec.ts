...
const SEARCH_PAGE = {
  ...
  data: {
    result: [
      { bvid: 'BV1H5gs6KE12', title: '想欣赏<em class="keyword">妾身</em>的舞姿吗？', author: 'UP甲', duration: '0:19', pic: 'http://i0.hdslb.com/bfs/archive/a.jpg' },
      { bvid: 'BV1nEgo6XEjr', title: '夜店热舞', author: 'UP乙', duration: '1:36', pic: '//i0.hdslb.com/bfs/archive/b.jpg' },
      { bvid: 'av12345', title: '非BV结果应被丢弃', author: 'x', duration: '0:10', pic: '' },
    ],
  },
}
const VIEW_PORTRAIT = { code: 0, data: { cid: 111, dimension: { width: 1080, height: 1920 } } }
const VIEW_LANDSCAPE = { code: 0, data: { cid: 222, dimension: { width: 1920, height: 1080 } } }
const PLAY = { code: 0, data: { durl: [{ url: 'https://upos-sz-estgcos.bilivideo.com/x.mp4' }] } }
...
describe('bilibili-shorts', () => {
  ...
  it('searches, portrait-preflights, and drops non-BV / landscape entries', async () => {
    ...
    const fetchMock = (async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url.includes('/view?bvid=BV1H5gs6KE12')) { views.push('portrait'); return jsonRes(VIEW_PORTRAIT) }
      if (url.includes('/view?bvid=BV1nEgo6XEjr')) { views.push('landscape'); return jsonRes(VIEW_LANDSCAPE) }
    ...
    expect(items[0]).toMatchObject({ bvid: 'BV1H5gs6KE12', cid: 111, durationSec: 19 })
  ...
  it('resolves mp4 play urls', async () => {
    ...
    const feed = new BiliShortsFeed(fetchMock)
    expect(await feed.play('BV1H5gs6KE12', 111)).toEqual(['https://upos-sz-estgcos.bilivideo.com/x.mp4'])
  })

  it('surfaces friendly errors', async () => {
    const dead = (async () => { throw new Error('net') }) as unknown as typeof fetch
