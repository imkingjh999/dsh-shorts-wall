...
const realPage = readFileSync(join(fixtures, "yt_search.fixture"), "utf8");
const realShortsPage = readFileSync(join(fixtures, "yt_shorts_search.fixture"), "utf8");
...
describe("YtFeed", () => {
  ...
  it("serves shorts pages with the shorts filter and keeps duration-less entries", async () => {
                    ...
                    contents: [
                      {
                        shortsLockupViewModel: {
                            ...
                            innertubeCommand: {
                              reelWatchEndpoint: {
                                ...
                                thumbnail: {
                                  thumbnails: [
                                    {
                                      url: "https://i.ytimg.com/vi/AAAAAAAAAAA/frame0.jpg",
                                    },
                                  ],
                                },
                              },
                          ...
                          overlayMetadata: {
                            primaryText: { content: "竖屏热舞" },
                            secondaryText: { content: "100次观看" },
                          },
                        },
                      },
                    ],
    ...
    const fetchMock = (async (input: RequestInfo | URL): Promise<Response> => {
      ...
      seen.push(url);
      return new Response(
        `<!doctype html><script>var ytInitialData = ${JSON.stringify(shortsFixture)};</script>`,
        { status: 200 },
      );
    }) as unknown as typeof fetch;
    const feed = new YtFeed(fetchMock);
  ...
  it("surfaces a friendly error when the page carries no data", async () => {
    const fetchMock = (async (): Promise<Response> =>
      new Response("<html>consent wall</html>")) as unknown as typeof fetch;
    const feed = new YtFeed(fetchMock);
